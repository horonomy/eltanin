...
| `B-M2-DEVFLOW-v2` | HORO-797/HORO-1278 | [`crates/eltanin-cli/tests/mvp2_dev_flow_e2e.rs`](../../../crates/eltanin-cli/tests/mvp2_dev_flow_e2e.rs) | [`B-M2-DEVFLOW.md`](B-M2-DEVFLOW.md) | F-M2-001, F-M2-002, F-M2-005, F-M2-006 |
...
| F-M2-001 — Trusted Compute Session | `B-M2-DEVFLOW-v2` — a real product gap found while originally building `B-M2-DEVFLOW-v1` (HORO-797) — `eltanin session start`'s session anchor died with that short-lived process, so no later `eltanin` invocation ever saw the session it established — was resolved by HORO-1278's session-anchor redesign (anchored to the caller's real POSIX session id instead of the connecting CLI peer). `f_m2_001_*` in `mvp2_dev_flow_e2e.rs` now proves a session established by one process is honored by later, genuinely separate `eltanin session list`/`eltanin run` invocations, that `eltanin session end` fails a later run closed, and that an expired session denies a later run and is honestly reported gone. See [`B-M2-DEVFLOW.md`](B-M2-DEVFLOW.md)'s "A real product gap found while building this scenario — resolved by HORO-1278" section. |
