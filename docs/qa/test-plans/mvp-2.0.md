...
| 3 | Protocol/parser fuzzing | DEFERRED | Unchanged from MVP 1.0 — `eltanin-protocol/tests/protocol_fail_closed.rs` remains the hand-written negative-input coverage; no structured fuzzing harness exists. Seven `DOMAIN_SCHEMA_VERSION` wire-shape additions (sessions, approvals, delegation, step-up, revocation-adjacent, shadow/audit, session-anchor redesign) since MVP 1.0 have not changed this gap's nature — deferred pending a driving incident, same as before. |
...
| 10 | Supply-chain/release integrity tests | REQUIRED | `cargo-deny` continues to run in CI on every PR, unchanged from MVP 1.0. **Known gap closed**: `origin/main`'s `34420fc` (CodeQL advanced setup covering Actions and Rust) and `177afa1` (CI workflow `GITHUB_TOKEN` restricted to `contents:read`), landed via PR #58/#59 (HORO-1270), are now both ancestors of `origin/next/mvp-2.0` (verified at this reconciliation's time of writing via `git merge-base --is-ancestor`, both true) — `next/mvp-2.0`'s CI now runs CodeQL and carries the hardened default `GITHUB_TOKEN` scope, no longer a stated gap. |
...
**Re-run for HORO-1278** (Trusted Compute Session identity redesign —
anchor on the POSIX sid with reject-only leader corroboration, rather
than the connecting CLI process): every scenario below was re-checked
against the redesigned `eltanin-core/src/session.rs` and
`eltanin-agent/src/authz/{mod.rs,session_state.rs}`. Two scenarios
changed as a result — **S1** (the defense mechanism changed, not merely
"still passes") and **S8** (split into S8a/S8b, unchanged, plus a new
**S8c** pinning the redesign's disclosed TTL-bounded-not-terminal-bounded
trade-off) — and **S3**/**S12** were confirmed unaffected with the
reasoning now stated in-line below and in each test's own doc comment.
The other seven scenarios do not depend on session-leader-liveness
semantics at all (verified: the redesign touches only the files named
above) and are unaffected.

| # | Scenario (HORO-797 wording) | Coverage | Defending test(s) |
|---|---|---|---|
| S1 | Process restart and PID reuse | Pre-existing (MVP 1.0) + **defense mechanism CHANGED (HORO-1278)** | `crates/eltanin-core/tests/identity_adversarial.rs::pid_reused_by_unrelated_process_is_reported_different_not_same`, `::same_pid_different_binary_disguised_as_restart_is_not_reported_same` (identity-comparison layer, unchanged). At the *session*-anchor layer, the defense changed: previously a `TrustedSession` was accidentally protected against PID/sid reuse only because ANY same-session detached process was killed the moment the CLI leader exited — i.e. the bug HORO-1278 fixed was itself standing in as S1's "defense." Post-redesign, S1 is defended for real by sid-recycle corroboration plus uid/host re-checks: `crates/eltanin-core/src/session.rs::validate_reports_anchor_recycled_when_leader_is_confirmed_different` (a confirmed *different*, live process now occupying the recorded sid denies as `AnchorRecycled`) together with `::absent_leader_observation_does_not_deny`/`::unobserved_leader_at_establishment_does_not_deny` (which pin the reject-only property: a merely-unobservable leader, the expected common case, never denies — only a confirmed contradiction does). This is a real before/after distinction worth stating plainly, not a re-derivation of the identity-adversarial tests above. |
| S2 | Binary replacement / identity change | Pre-existing (MVP 1.0 execve substitution) + MVP 2.0 digest binding — unchanged by HORO-1278 | `crates/eltanin-agent/tests/authz_release.rs::an_execve_substitution_is_refused_even_with_the_same_pid_and_start_token` (MVP 1.0); `authz_approval.rs::a_changed_executable_digest_is_reevaluated_and_denied` (F-M2-002). Neither test depends on session-leader-liveness semantics — the session-anchor redesign is confined to `eltanin-core/src/session.rs`, `eltanin-agent/src/authz/{mod.rs,session_state.rs}` (verified: `grep -rl LeaderCorroboration\|AnchorRecycled` across the workspace), none of which this scenario's tests touch. |
| S3 | Same-user unrelated process attempting compute | New (PR #63) — **unchanged by HORO-1278** | `authz_session.rs::s3_a_same_session_unrelated_process_is_admitted_with_no_further_defense` — `PINS_LIMITATION`: pins ADR 0009's `postinstall` trade-off and ADR 0010 disclosure 2 as real, not hypothetical. Confirmed still accurate post-redesign and now says so in its own doc comment: HORO-1278's new uid/host checks in `membership()` only reject a *different* uid/host than the session owner's, and S3's scenario is specifically the *same* uid — so the gap this test pins is untouched, and the test passes unmodified. |
| S4 | Descendant privilege laundering | New (PR #63) — unchanged by HORO-1278 | `crates/eltanin-core/tests/delegation_admission.rs::uid_transition_inside_ancestry_span_is_invisible_to_admission` — `PINS_LIMITATION`: `ProcessAncestor` has no uid field, so a uid transition strictly inside the requester→holder ancestry span is invisible to admission (ADR 0011 disclosure 3). No session-leader/anchor concept involved. |
| S5 | Python/Node/shell interpreter misuse | New (PR #63) — unchanged by HORO-1278 | `authz_approval.rs::s5_approving_an_interpreter_admits_a_later_invocation_with_a_different_script` — `PINS_LIMITATION`: ADR 0010's AC4-partial, the argv-invisibility gap. No session-leader/anchor concept involved. |
| S6 | Malicious/unexpected child from a trusted agent/tool | New (PR #63) — unchanged by HORO-1278 | `authz_delegation.rs::s6_a_depth_one_descendant_with_an_unexpected_executable_is_still_admitted` — `PINS_LIMITATION`: ADR 0011 disclosure 5 (trust-transition detection is path-only, and a depth-1 descendant's transition-marker scan span is empty). No session-leader/anchor concept involved. |
| S7 | Downloaded/temp executable | New (PR #63) — unchanged by HORO-1278 | `authz_step_up.rs::s7_agent_level_untrusted_execution_path_composes_through_the_real_gate_chain` — `PROVES_DEFENSE`: `RiskSignal::UntrustedExecutionPath` composes through the real gate chain as a step-up-shaped refusal (ADR 0012). No session-leader/anchor concept involved. |
| S8 | Detached persistent daemon | New (PR #63) split into three sub-scenarios by HORO-1278 | **S8a** (a `setsid()`-detached process — new POSIX session — correctly refused/flagged): `authz_step_up.rs::s8a_a_real_detached_process_fires_detached_execution_through_the_real_gate_chain` — `PROVES_DEFENSE` (fires `RiskSignal::DetachedExecution` through the real gate chain). **S8b** (same process, default config, no step-up configured): `authz_step_up.rs::s8b_pins_that_under_the_default_config_a_detached_process_is_admitted_anyway` — `PINS_LIMITATION` (detection is opt-in; admitted with zero friction unless `ApprovalRequirement`/`StepUpPolicy` are explicitly configured). **S8c** (NEW, HORO-1278): a same-session process that never calls `setsid()` (a double-fork-shaped detached daemon that keeps the leader's sid) is admitted for the full session TTL once the leader dies, bounded only by expiry: `authz_session.rs::s8c_a_same_session_detached_process_is_bounded_by_ttl_not_by_terminal_lifetime` — `PINS_TRADEOFF`, explicitly **not a regression**: this is the HORO-1278 redesign's disclosed trade-off (`SessionAuthority::validate`'s own doc: "what now bounds a `TrustedSession`'s lifetime... is TTL/expiry alone, not leader liveness... exactly the founder's 'explicit TTL/expiry' requirement, made structural rather than incidental"). The test proves the TTL boundary directly (admitted immediately after leader death, denied once the TTL elapses), distinguishing correct TTL-bounded admission from the wrong "no expiry at all" shape. |
| S9 | Policy/session revoke during long-running work | Pre-existing (MVP 2.0) — unchanged by HORO-1278 | `authz_session.rs::ac3_terminating_a_session_prevents_new_lease_issuance_under_required`, `::ac3_terminating_a_session_revokes_leases_issued_under_it` (F-M2-001). Explicit `TerminateSession` semantics are untouched by the anchor redesign. |
| S10 | Lease expiry/renewal races | Pre-existing (MVP 2.0) — unchanged by HORO-1278 | `authz_concurrency.rs` (capacity-overshoot, shared-resource release/request race, compensating-revoke leak, duplicate-lease-id checks under real concurrency via `std::thread::scope`), `authz_lifecycle.rs::an_expired_lease_is_pruned_and_no_longer_releasable` (F-M2-005). Lease-layer expiry, independent of session-leader semantics. |
...
| S12 | Agent restart/reboot/offline-policy cases | New (PR #63) — **unchanged by HORO-1278**, re-verified | `authz_session.rs::s12_a_durable_approval_survives_restart_while_the_session_and_its_lease_do_not` — `PROVES_DEFENSE` (design-honesty): proves the restart asymmetry directly (a durable `Remember` approval survives an agent restart; the Trusted Compute Session and its lease do not). Re-verified post-HORO-1278: `SessionState`/`SessionAuthority` gained no persistence (still an in-memory `BTreeSet<u64>` of terminated sequences plus a fresh `IssuerInstanceId` per restart — verified by reading `crates/eltanin-core/src/session.rs`/`crates/eltanin-agent/src/authz/session_state.rs`), so a session still does not survive an agent restart and this test is unaffected. |
...
| F-M2-001/002/003/004/005 operator-configurability via `eltanin-agentd` | **CLOSED.** PR #65 exposed session/approval/revocation as `eltanin-agentd` environment-variable configuration (`ELTANIN_AGENT_SESSION_REQUIRED`, `_APPROVAL_REQUIRED`+`_APPROVAL_STORE`, `_REVOCATION_REQUIRED`), and PR #71 (HORO-1278) closed the remaining delegation/step-up gap via `ELTANIN_AGENT_GATE_CONFIG` (a JSON file reaching `AuthorizationConfig::with_delegation`/`with_step_up`). Every MVP 2.0 gate is now operator-configurable, matching F-M2-006's shadow mode (`ELTANIN_AGENT_MODE`) precedent. |
