...
| 4 | Component/integration tests | REQUIRED | The bulk of this workspace's test suite: `crates/*/tests/*.rs` integration tests, including real-socket end-to-end tests (`eltanin-agent/tests/authz_end_to_end.rs`, `eltanin-cli/tests/canonical_e2e.rs`) that exercise the real binaries together, not just library calls. The Apple Silicon adapter's discovery path (`crates/eltanin-apple/tests/non_macos_fallback.rs`, F-M1-010/HORO-1012) exercises both the real `macos` discovery path (validated on a real macOS dev machine, not run by this repo's `ubuntu-latest` CI) and the `Unsupported` fallback every other target actually runs in CI — same dual-arm pattern as `eltanin-linux/tests/peer_credential.rs`. |
