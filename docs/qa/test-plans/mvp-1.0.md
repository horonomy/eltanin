...
| 6 | Adversarial/security regression corpus | REQUIRED | Broad and real — see "North Star invariant → defending tests" below for the indexed list. Not a separate directory; these are ordinary `crates/*/tests/*.rs` files named for what they defend against (spoof, replay, self-asserted-identity, PID reuse, execve substitution, etc.). `crates/eltanin-backend/tests/capability_support_states.rs` (HORO-1011) defends the compatibility-vs-protection distinction ADR 0006 introduces: `Partial`/`NotEvaluated` device-enforce support and mere `ControlledLaunch` support both provably never produce `EnforcementResult::Allowed`. |
